...
const ArtPayment = () => {
    ...
    if (location.state && (location.state as any).post) {
      ...
      if (receivedPost.tradeDTO) {
          ...
          // 여기서는 tradeDTO.highestBid를 우선 사용하고, 없으면 nowBuy를 사용하도록 로직 변경
           const finalPrice = receivedPost.tradeDTO.highestBid ?? receivedPost.tradeDTO.nowBuy ?? 0;
  ...
  const processPayment = async () => {
      ...
      try {
          ...
          const response = await fetch(`http://localhost:8080/ourlog/trades/${tradeId}/nowBuy`, { // TODO: 백엔드 결제 완료 API로 변경 필요
          ...
          <div className="artwork-order-info">
            <div className="artwork-thumbnail">
              ...
              {mainImagePicture && mainImagePicture.originImagePath ? (
                <img
                  src={mainImagePicture.originImagePath.startsWith('/ourlog') ? `http://localhost:8080${mainImagePicture.originImagePath}` : `http://localhost:8080/ourlog/picture/display/${mainImagePicture.originImagePath}`}
        ...
        <div className="payment-info-section">
          ...
          <div className="policy-detail">
            ...
            <ul>
              ...
              <li>통관 운송료/보험료는 기기마다 다른 차액 책정은 회원/작가의 사정 하에 가격 차이로 책정되며, 작품 충 고러가 기타로 책정 되었을 나사에만 추적이어니 광매리자 리객 어깨 채번을 그재 대어성 리을 초후를 확인하 뷀바바니다.</li>
            ...
            {postData.pictureDTOList
              ...
              .map((picture, index) => {
                const imageUrl = picture.originImagePath
                  ...
                  ? picture.originImagePath.startsWith('/ourlog') ? `http://localhost:8080${picture.originImagePath}` : `http://localhost:8080/ourlog/picture/display/${picture.originImagePath}`
                  : null;
