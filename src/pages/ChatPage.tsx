...
const ChatPage: React.FC = () => {
  ...
  const [publicChannelsError, setPublicChannelsError] = useState<string | null>(
    null
  );
  ...
  const [openMenuChannelUrl, setOpenMenuChannelUrl] = useState<string | null>(
    null
  );
  ...
  const fetchSendbirdAuthInfo = async () => {
    ...
    const token = tokenWithPrefix.startsWith("Bearer ")
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error(
          "Failed to fetch Sendbird auth info:",
          response.status,
          errorData
        );
        const errorMessage =
      ...
      if (userString) {
        try {
          ...
          console.error(
            "Failed to parse user info from localStorage for userId",
            e
          );
        }
      }
      ...
      if (!backendUserId) {
        throw new Error(
          "Could not determine Sendbird User ID (backend userId)."
        );
      }
      ...
      setError(
        `채팅 시스템 초기화 실패: ${e.message || "알 수 없는 오류 발생"}`
      );
      setLoading(false);
      return null;
    }
  ...
  useEffect(() => {
    const initializeAndConnectSendbird = async () => {
      ...
      try {
        ...
        const user = await sbInstance.current.connect(
          userInfo.userId,
          userInfo.sendbirdAccessToken
        );
        ...
        const channelListQuery: GroupChannelListQuery =
          sbInstance.current.groupChannel.createMyGroupChannelListQuery({
            limit: 100,
          });
              ...
              if (otherUser) {
                try {
                  ...
                  if (!isNaN(backendUserId)) {
                    ...
                    console.warn(
                      "Failed to parse Sendbird userId to integer for fetching profile:",
                      otherUser.userId
                    );
                    return null;
                  }
                  ...
                  console.error(
                    `Failed to fetch profile for user ${otherUser.userId}:`,
                    profileError
                  );
                  return null;
                }
        ...
        const fetchedProfiles = await Promise.all(
          profilePromises.filter((p) => p !== null)
        ); // Filter out nulls before Promise.all
        ...
        const channelHandler = new GroupChannelHandler({
          onMessageReceived: (channel: BaseChannel, message: BaseMessage) => {
            ...
            if (channel.isGroupChannel()) {
              ...
              if (message.messageType === "user") {
                ...
                console.log(
                  `[${groupChannel.url}] onMessageReceived: 새 메시지 수신 - ${userMessage.messageId}`,
                  userMessage
                );
                ...
                if (
                  ...
                  currentChannel?.url !== groupChannel.url
                ) {
                  const senderProfile =
                    userProfiles[userMessage.sender?.userId || ""];
                  const senderNickname =
                    senderProfile?.nickname ||
                    userMessage.sender?.nickname ||
                    userMessage.sender?.userId ||
                    "알 수 없는 사용자";
                  ...
                  setChatNotification({
                    ...
                    senderProfileImage:
                      senderProfile?.thumbnailImagePath ||
                ...
                console.log(
                  `Received non-user message of type: ${message.messageType}`
                );
                const formattedMessage: ChatMessage = {
                  sender: "System",
                  message:
              ...
              console.warn(
                "Received message from a non-group channel:",
                channel
              );
            }
          },
                    ...
                    fetchProfile(backendUserId)
                      .then((profile) =>
                        setUserProfiles((prev) => ({
                          ...prev,
                          [otherUser.userId]: profile,
                        }))
                      )
                      .catch((profileError) =>
                        console.error(
                          `Failed to fetch profile for user ${otherUser.userId} on channel change:`,
                          profileError
                        )
                      );
                  }
          ...
          onChannelDeleted: (channelUrl: string, channelType: string) => {
            ...
            setChannels((prevChannels) =>
              prevChannels.filter((ch) => ch.url !== channelUrl)
            );
          ...
          onMessageUpdated: (channel: BaseChannel, message: BaseMessage) => {
            ...
            if (channel.isGroupChannel()) {
              ...
              if (message.messageType === "user") {
                ...
                console.log(
                  `[${groupChannel.url}] onMessageUpdated: 메시지 업데이트 - ${userMessage.messageId}`,
                  userMessage
                );
                ...
                try {
                  if (
                    ...
                    if (customData.isPaymentComplete !== undefined) {
                      formattedUpdatedMsg.isPaymentComplete =
                        customData.isPaymentComplete;
                    }
                    if (customData.isPaymentFormVisible !== undefined) {
                      formattedUpdatedMsg.isPaymentFormVisible =
                        customData.isPaymentFormVisible;
                    }
                  }
                } catch (e) {
                  console.error(
                    "Failed to parse updated message data in handler:",
                    e
                  );
                }
              }
        ...
        if (sbInstance.current) {
          sbInstance.current.groupChannel.addGroupChannelHandler(
            channelHandlerId,
            channelHandler
          );
          console.log(
            "Sendbird channel handler added with ID:",
            channelHandlerId
          );
        }
      } catch (error: any) {
        ...
        setError(
          `채팅 시스템 초기화 실패: ${error.message || "알 수 없는 오류 발생"}`
        );
      } finally {
        setLoading(false);
      }
    ...
    return () => {
      ...
      if (sbInstance.current) {
        sbInstance.current.groupChannel.removeGroupChannelHandler(
          channelHandlerId
        );
        console.log(
          "Sendbird channel handler removed with ID:",
          channelHandlerId
        );
        ...
        if (typeof sbInstance.current.disconnect === "function") {
          ...
          console.warn(
            "Sendbird disconnect method not found on sbInstance.current."
          );
        }
      }
  ...
  useEffect(() => {
    ...
    if (state?.targetUserId) {
      console.log(
        `Detected targetUserId in state: ${state.targetUserId}. Setting initialTargetUserId state.`
      );
  ...
  useEffect(() => {
    console.log(
      "Checking conditions for starting chat from initialTargetUserId state..."
    );
    ...
    if (isSendbirdInitialized && currentUser && initialTargetUserId) {
      console.log(
        `Conditions met: isSendbirdInitialized=${isSendbirdInitialized}, currentUser=${!!currentUser}, initialTargetUserId=${initialTargetUserId}. Attempting to start chat.`
      );
      ...
      console.log(
        "Conditions not met for starting chat from initialTargetUserId state.",
        {
  ...
  useEffect(() => {
    const fetchOtherUserProfile = async () => {
      ...
      if (
        ...
        (currentChannel as GroupChannel).members &&
        currentUser
      ) {
  ...
  useEffect(() => {
    const fetchPublicChannels = async () => {
      ...
      try {
        ...
        const publicChannelListQuery: PublicGroupChannelListQuery =
          sbInstance.current.groupChannel.createPublicGroupChannelListQuery({
            limit: 20,
            includeEmpty: true,
          });
        ...
        setPublicChannelsError(
          `공개 채널 목록 로딩 실패: ${e.message || "알 수 없는 오류"}`
        );
      } finally {
    ...
    if (sbInstance.current) {
      try {
        const channel: GroupChannel =
          await sbInstance.current.groupChannel.getChannel(channelUrl);
        ...
        return new Promise<void>((resolve, reject) => {
          const collectionHandler: MessageCollectionEventHandler = {
            onMessagesAdded: (context, channel, messages) => {
              console.log(
                `[${channel.url}] MessageCollection: Messages added`,
                messages
              );
              const formattedMessages = messages.map((msg) => {
                ...
                try {
                  ...
                  console.error(
                    "Failed to parse message data in collection handler:",
                    e
                  );
                }
                return baseMessage;
            ...
            onMessagesDeleted: (context, channel, messageIds) => {
              console.log(
                `[${channel.url}] MessageCollection: Messages deleted`,
                messageIds
              );
              setMessagesByUser((prev) => ({
                ...prev,
                [channel.url]: (prev[channel.url] || []).filter((msg) => {
                  ...
                  return !messageIds
                    .map(String)
                    .includes(String(messageIdToRemove));
                }),
              }));
              console.log(
                `[${channel.url}] MessageCollection: messagesByUser 상태에서 삭제 완료.`,
                messagesByUser
              );
            },
            onChannelUpdated: (
              ...
              console.log(
                `[${channel.url}] MessageCollection: Channel updated`,
                channel
              );
              setChannels((prevChannels) =>
                prevChannels.map((ch) =>
            ...
            onChannelDeleted: (
              ...
              console.log(
                `[${channelUrl}] MessageCollection: Channel deleted`,
                channelUrl
              );
            },
          };
          ...
          collection
            ...
            .onCacheResult((messages, error) => {
              console.log(
                `[${channel.url}] MessageCollection: Initial messages from cache`,
                messages,
                error
              );
              if (Array.isArray(messages) && messages.length > 0) {
                ...
                setMessagesByUser((prev) => ({
                  ...prev,
                  [channel.url]: formattedMessages.sort(
                    (a, b) => (a.createdAt || 0) - (b.createdAt || 0)
                  ),
                }));
                console.log(
                  `[${channel.url}] MessageCollection: messagesByUser 상태에 캐시 메시지 설정 완료.`,
                  messagesByUser
                );
              } else if (Array.isArray(messages)) {
                // 캐시 결과가 없거나 비어있는 경우에도 상태를 초기화합니다.
                setMessagesByUser((prev) => ({ ...prev, [channel.url]: [] }));
                console.log(
                  `[${channel.url}] MessageCollection: 캐시 메시지 없음 또는 비어있음. 상태 초기화.`,
                  messagesByUser
                );
              }
            })
            .onApiResult((messages, error) => {
              ...
              console.log(
                `[${channel.url}] MessageCollection: Initial API load result - messages:`,
                messages,
                "error:",
                error
              );
              ...
              if (
                ...
                // 실제 Sendbird 오류가 아닌 경우 (메시지 목록이 error로 온 경우 포함)
                let apiMessages: BaseMessage[] = []; // API 결과를 저장할 배열 초기화
                ...
                if (apiMessages.length > 0) {
                  ...
                  // apiMessages 배열이 비어있는 경우
                  console.log(
                    `[${channel.url}] MessageCollection: Initial API load resulted in no messages.`
                  );
                }
        ...
        console.error(
          "handleOpenChatModal failed to get channel or initialize MessageCollection:",
          err
        );
        const errorMessage =
          err?.message ||
  ...
  const handleSendMessage = () => {
    ...
    currentChannel
      ...
      .onFailed((error) => {
        ...
        setError(
          `메시지 전송 실패: ${error.message || "알 수 없는 오류 발생"}`
        );
      });
  };
  ...
  const togglePaymentForm = async (messageId: number | undefined) => {
    ...
    const messageToUpdate = messagesByUser[currentChannel!.url]?.find(
      (msg) => msg.messageId === messageId
    );
    if (
      !messageToUpdate ||
      messageToUpdate.customType !== "payment_request" ||
      !messageToUpdate.data
    ) {
      console.warn(
        "Message not found or is not a payment request message for toggle."
      );
      return;
    }
    ...
    try {
      ...
      const updatedMessage = await currentChannel!.updateUserMessage(
        messageId,
        {
      ...
      console.log(
        "Payment form visibility updated via Sendbird message update:",
        updatedMessage
      );
    } catch (error: any) {
      console.error("Failed to update payment form visibility:", error);
      setError(
        `결제 폼 상태 업데이트 실패: ${error.message || "알 수 없는 오류 발생"}`
      );
    }
  };
  ...
  const handlePaymentSubmit = async (messageId: number | undefined) => {
    ...
    const messageToComplete = messagesByUser[currentChannel.url]?.find(
      (msg) => msg.messageId === messageId
    );
    if (
      !messageToComplete ||
      messageToComplete.customType !== "payment_request" ||
      !messageToComplete.data
    ) {
      console.warn(
        "Message not found or is not a payment request message for completion."
      );
      setCardNumber("");
    ...
    try {
      ...
      console.log(
        "Attempting to update payment status for message:",
        messageId,
        "with data:",
        updatedData
      );
      ...
      console.log(
        "Payment status updated via Sendbird message update:",
        updatedMessage
      );
      setCardNumber("");
      ...
      if (users && users.length > 0) {
        ...
        if (!isNaN(backendUserId)) {
          try {
            ...
            setUserProfiles((prev) => ({
              ...prev,
              [foundSendbirdUser.userId]: profile,
            }));
            ...
            console.error(
              `Failed to fetch profile for found user ${foundSendbirdUser.userId}:`,
              profileError
            );
          }
        } else {
  ...
  const handleStartNewChat = async (targetSendbirdUserId: string) => {
    if (!sbInstance.current || !currentUser || !targetSendbirdUserId) {
      console.error(
        "Cannot start chat: SDK not initialized or target user ID missing."
      );
    ...
    try {
      console.log(
        `Attempting to create or get channel with user ID: ${targetSendbirdUserId}`
      );
      const params = {
      ...
      const newChannel: GroupChannel =
        await sbInstance.current.groupChannel.createChannel(params);
      ...
      setChannels((prevChannels) => {
        ...
        return [
          newChannel,
          ...prevChannels.filter((ch) => ch.url !== newChannel.url),
        ];
      });
      ...
      if (
        newChannel.isGroupChannel() &&
        (newChannel as GroupChannel).members &&
        currentUser
      ) {
        const groupChannel = newChannel as GroupChannel; // Cast to GroupChannel
        if (groupChannel.memberCount === 2) {
          // Now safe to access memberCount
          const otherUser = groupChannel.members.find(
            (member) => member.userId !== currentUser.userId
          );
          if (otherUser) {
            ...
            if (!isNaN(backendUserId)) {
              fetchProfile(backendUserId)
                ...
                .catch((profileError) =>
                  console.error(
                    `Failed to fetch profile for user ${otherUser.userId} after starting chat:`,
                    profileError
                  )
                );
            }
      ...
      console.log(
        `handleOpenChatModal completed for channel: ${newChannel.url}. Proceeding with navigation.`
      );
  ...
  const handleCloseChannel = async (channelUrl: string) => {
    if (!sbInstance.current || !currentUser) {
      console.error(
        "Cannot close channel: SDK not initialized or user not found."
      );
    ...
    if (!channelToClose) {
      setError("채널 닫기 실패: 채널을 찾을 수 없습니다.");
      setOpenMenuChannelUrl(null);
      return;
    }
  ...
  const handleDocumentClick = (event: MouseEvent) => {
    ...
    // 클릭된 요소가 메뉴 버튼인지 확인
    const menuButtonElement = (event.target as HTMLElement).closest(
      ".channel-menu-button"
    );
    // 클릭된 요소가 보류 작업 확인/취소 버튼인지 확인
    const actionButtonElement = (event.target as HTMLElement).closest(
      ".action-confirm-button, .action-cancel-button"
    );

    // 만약 클릭된 요소가 메뉴 버튼 또는 액션 버튼이라면, 메뉴를 닫지 않고 바로 종료
    if (menuButtonElement || actionButtonElement) {
      console.log(
        "Click originated from menu button or action button. Not closing menu."
      );
      return;
    }
    ...
    if (
      ...
      console.log(
        "Click inside menu, menu is closed, or click on action buttons."
      ); // 디버그 로그
    }
  };
  ...
  const handleLeaveChannel = async (channelUrl: string) => {
    if (!sbInstance.current || !currentUser) {
      console.error(
        "Cannot leave channel: SDK not initialized or user not found."
      );
    ...
    if (!channelToLeave) {
      console.warn(
        "Attempted to leave channel that does not exist in state:",
        channelUrl
      );
  ...
  const handleToggleMuteNotifications = async (channelUrl: string) => {
    if (!sbInstance.current || !currentUser) {
      console.error(
        "Cannot toggle notifications: SDK not initialized or user not found."
      );
    ...
    if (!channelToToggle) {
      console.warn(
        "Attempted to toggle notifications for channel that does not exist in state:",
        channelUrl
      );
    ...
    try {
      console.log(
        `Attempting to toggle notifications for channel: ${channelUrl}`
      );
      ...
      const newOption =
        currentOption === PushTriggerOption.DEFAULT ||
        currentOption === PushTriggerOption.ALL
          ? PushTriggerOption.OFF
          : PushTriggerOption.DEFAULT;

      console.log(
        `Current push trigger option: ${currentOption}. Setting to: ${newOption}`
      );

      await groupChannel.setMyPushTriggerOption(newOption);

      console.log(
        "Successfully toggled notifications for channel:",
        channelUrl,
        "New option:",
        newOption
      );
  ...
  const handleRenameChannel = async (channelUrl: string) => {
    if (!sbInstance.current || !currentUser) {
      console.error(
        "Cannot rename channel: SDK not initialized or user not found."
      );
    ...
    if (
      !channelToRename ||
      !(channelToRename as GroupChannel).creator ||
      (channelToRename as GroupChannel).creator?.userId !== currentUser.userId
    ) {
      console.warn(
        "Attempted to rename channel without being the creator or channel not found."
      );
    ...
    console.log(
      `Setting pending rename action for channel: ${channelUrl} with new name: "${newName}"`
    );
  ...
  const handleConfirmAction = async () => {
    ...
    const channel = channels.find((ch) => ch.url === channelUrl) as
      | GroupChannel
      | undefined;
    ...
    try {
      console.log(
        `Executing pending action: ${type} for channel: ${channelUrl}`
      );
      switch (type) {
        case "leave":
          await channel.leave();
      ...
      setError(
        `작업 실행 실패 (${type}): ${e.message || "알 수 없는 오류 발생"}`
      );
    } finally {
  ...
  const handleCancelAction = () => {
    console.log(
      `Cancelling pending action for channel: ${pendingAction?.channelUrl}`
    );
    setPendingAction(null); // 상태 초기화
  ...
  const showRenameAndDeleteMenu = (
    channel: GroupChannel | null,
    user: User | null
  ): boolean => {
    // 공개 채널이면 누구나 닫기/이름바꾸기 메뉴가 보이게
    return !!(channel?.isGroupChannel() && (channel as GroupChannel).isPublic);
  };
    ...
    <div className="chat-page">
        ...
        <div className="chat-list-section">
          ...
          {/* {isCreateChannelModalVisible && (
            <div className="create-channel-modal">
              ...
              />
              <button
                className="create-channel-submit-button"
              ...
              <button
                className="create-channel-cancel-button"
                onClick={() => setIsCreateChannelModalVisible(false)}
              >
                취소
              </button>
            </div>
          ...
          <div className="new-chat-section">
            ...
            <div className="user-search-row">
              ...
              <button
                ...
                disabled={isSearching || !isSendbirdInitialized}
              >
            ...
            {foundUser && (
              <div className="found-user">
                ...
                <img
                  src={
                    userProfiles[foundUser.userId]?.thumbnailImagePath ||
          ...
          <div className="my-chat-list-section">
            ...
            <ul className="chat-list">
              {channels.map((channel) => {
                ...
                const isOneToOneChannel =
                  isGroup &&
                  (channel as GroupChannel).memberCount === 2 &&
                  currentUser;
                const otherUser =
                  isOneToOneChannel && (channel as GroupChannel).members // members 속성 안전 접근
                    ? (channel as GroupChannel).members.find(
                        (member) => member.userId !== currentUser.userId
                      ) || null
                    : null;
                const otherUserProfile = otherUser
                  ? userProfiles[otherUser.userId]
                  : undefined;

                // 표시 이름 결정: 1:1 채널이면 상대방 이름, 아니면 채널 이름
                const displayName = isOneToOneChannel
                  ? otherUser?.nickname ||
                    otherUser?.userId ||
                    "알 수 없는 사용자"
                ...
                const displayImage =
                  otherUserProfile?.thumbnailImagePath ||
                ...
                const isChannelCreator =
                  currentUser &&
                  isGroup &&
                  (channel as GroupChannel).creator?.userId ===
                    currentUser.userId;

                // ✅ 1:1 채널에서는 이름 바꾸기 및 채널 닫기 메뉴를 표시하지 않음 - 헬퍼 함수 사용
                const canShowRenameAndDeleteMenu = showRenameAndDeleteMenu(
                  channel,
                  currentUser
                );
                ...
                const isNotificationsMuted =
                  isGroup &&
                  (channel as GroupChannel).myPushTriggerOption ===
                    PushTriggerOption.OFF;
                ...
                return (
                  <li
                    ...
                    onClick={() =>
                      !isPending && handleOpenChatModal(channel.url)
                    }
                    className={`${
                      openMenuChannelUrl === channel.url ? "menu-open" : ""
                    } ${isPending ? "pending-action" : ""}`} // 메뉴 열림 및 보류 작업 상태 클래스 추가
                  >
                    ...
                    {isGroup && !isPending && (
                      <span
                        ...
                        title={isNotificationsMuted ? "알림 꺼짐" : "알림 켜짐"}
                      >
                        {/* 알림 꺼짐 상태일 때만 이미지 표시 */}
                        {isNotificationsMuted && (
                          <img
                            src="/images/chat-muted.png"
                            alt="알림 꺼짐"
                            style={{
                              width: "25px",
                              height: "25px",
                    ...
                    {isGroup &&
                      (isPending ? (
                        ...
                        <div className="action-buttons">
                          ...
                          <button
                            className="chat-action-confirm-button"
                          ...
                          <button
                            className="chat-action-cancel-button"
                        ...
                        <button
                          ...
                          onClick={(e) => toggleSideMenu(channel.url, e)}
                        >
                          ⋮ {/* 점 3개 아이콘 */}
                        </button>
                      ))}
            ...
            <div className="chat-modal">
              ...
              <div className="chat-header">
                <div className="chat-header-left">
                  ...
                  {
                    (currentChannel as GroupChannel)?.isGroupChannel() &&
                    (currentChannel as GroupChannel).memberCount === 2 &&
                    currentUser &&
                    (currentChannel as GroupChannel).members // 현재 채널이 GroupChannel이고 1:1 채널이며 사용자 정보가 있을 때
                      ? // 현재 채널의 상대방 프로필 정보 가져오기
                        (currentChannel as GroupChannel).members.find(
                          (member) => member.userId !== currentUser.userId
                        )?.nickname ||
                        (currentChannel as GroupChannel).members.find(
                          (member) => member.userId !== currentUser.userId
                        )?.userId ||
                        "알 수 없는 사용자" // 상대방 닉네임 또는 ID
              ...
              <div className="chat-messages">
                    ...
                    !msg.hidden && (
                      <div
                        ...
                        className={`message-row ${
                          msg.sender === currentUser.userId ? "me" : "you"
                        }`}
                      >
                        ...
                        {msg.sender !== currentUser.userId && (
                          ...
                          <img
                            src={
                              userProfiles[msg.sender]?.thumbnailImagePath ||
                              "/profile-placeholder.jpg"
                            }
                            alt="상대방"
                            className="profile-icon"
                            ...
                            <div className="payment-card">
                              ...
                              {!msg.isPaymentFormVisible &&
                                !msg.isPaymentComplete && (
                                  <button
                                    onClick={() =>
                                      togglePaymentForm(msg.messageId)
                                    }
                ...
                {messagesByUser[currentChannel.url]?.length === 0 &&
                  !loading &&
                  !error && <p>메시지가 없습니다.</p>}
              </div>
              <div className="chat-input-area">
                ...
                <button
                  ...
                  disabled={loading || !currentChannel}
                >
                  전송
                </button>
                <button
      ...
      {openMenuChannelUrl && menuPosition && (
        ...
        >
          ...
          {(() => {
            const targetChannel =
              channels.find((ch) => ch.url === openMenuChannelUrl) || null;
            if (!targetChannel) {
              console.warn(
                `Could not find channel with URL: ${openMenuChannelUrl} to render menu items.`
              ); // 디버그 로그
              return null; // 채널을 찾을 수 없으면 메뉴 항목을 렌더링하지 않음
            }
            const canShowLeave = showLeaveMenu(targetChannel, currentUser);
            const canShowRenameAndDelete = showRenameAndDeleteMenu(
              targetChannel,
              currentUser
            );
            ...
            return (
              <Fragment>
                ...
                <li
                  onClick={() =>
                    handleToggleMuteNotifications(openMenuChannelUrl)
                  }
                >
                  알림 설정 변경
                </li>
